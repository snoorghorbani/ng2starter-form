...
var EditFormEffects = /** @class */ (function () {
    function EditFormEffects(actions$, router, service) {
        ...
        this.EditForm$ = this.actions$
            ...
            .pipe(operators_1.map(function (action) { return action.payload; }), operators_1.map(function (data) { return new edit_form_actions_1.EditFormStartAction(data); }));
        this.EditFormStart$ = this.actions$
            .ofType(edit_form_actions_1.EditFormActionTypes.EDIT_FORM_START)
            .pipe(operators_1.map(function (action) { return action.payload; }), operators_1.switchMap(function (data) { return _this.service.update(data); }), operators_1.map(function (formSchema) { return new edit_form_actions_1.EditFormSucceedAction(formSchema); }), operators_1.catchError(function () { return Observable_1.Observable.of(new edit_form_actions_1.EditFormFailedAction()); }));
        this.UpdateFormsListStart$ = this.actions$
            .ofType(edit_form_actions_1.EditFormActionTypes.EDIT_FORM_SUCCEED)
            .pipe(operators_1.map(function (action) { return action.payload; }), operators_1.map(function (formSchema) { return new list_1.UpdateFormSchemaAction(formSchema); }));
    }
